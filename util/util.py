...
def tensor2gray(label_tensor, imtype=np.uint8):
    print(label_tensor.size())
    image_numpy = []
    label_tensor = label_tensor.cpu()
    image_numpy = label_tensor.detach().numpy()
    maxi = image_numpy.max()
    image_numpy = image_numpy*255./maxi
    _, a1, a2, a3 = image_numpy.shape
    print(image_numpy.shape)
    image_numpy = np.reshape(image_numpy, (a1, a2, a3))
    image_numpy = np.transpose(image_numpy, [1,2,0])
    image_numpy = np.squeeze(image_numpy,axis=2)
    return image_numpy.astype(imtype)

def save_image(image_numpy, image_path, create_dir=False, is_lbp=False):
    if create_dir:
    ...
    #     image_pil = Image.fromarray(image_numpy.reshape(h ,w), 'L')
